...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams AS t LEFT JOIN games AS g1 ON t.team_id = g1.winner_id FULL JOIN games AS g2 ON t.team_id = g2.opponent_id  WHERE (g1.year = 2014 AND g1.round = 'Eighth-Final') OR  (g2.year= 2014 AND g2.round = 'Eighth-Final') ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(name) FROM teams AS t LEFT JOIN games AS g ON t.team_id = g.winner_id WHERE g.round = 'Eighth-Final'  ORDER BY name")"
